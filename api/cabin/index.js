...
/**
 * Route that retrieves all CABIN
 */

// 50 is view CABIN tab so remove in needed
router.get("/getAllCabin", auth.authenticateRequest(50), multer().none(), async (req, res) => { // 50 is view CABIN tab so remove in needed
    ...
    jwt.verify(req.headers.logintoken, config.privateKey, (err, decoded) => {
        ...
        connectionPool.query(config.queries.getAllCabin, (err, response) => {
            if (err) {
