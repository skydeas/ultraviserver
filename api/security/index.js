...
/**
 * Route that retrieves all SECURITY
 */

// 51 is view SECURITY tab so remove in needed
router.get("/getAllSecurity", auth.authenticateRequest(51), multer().none(), async (req, res) => { // 51 is view SECURITY tab so remove in needed
    ...
    jwt.verify(req.headers.logintoken, config.privateKey, (err, decoded) => {
        ...
        connectionPool.query(config.queries.getAllSecurity, (err, response) => {
            if (err) {
