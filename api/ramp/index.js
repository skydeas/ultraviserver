...
/**
 * Route that retrieves all RAMP
 */

// 49 is view RAMP tab so remove in needed
router.get("/getAllRamp", auth.authenticateRequest(49), multer().none(), async (req, res) => { // 49 is view RAMP tab so remove in needed
    ...
    jwt.verify(req.headers.logintoken, config.privateKey, (err, decoded) => {
        ...
        connectionPool.query(config.queries.getAllRamp, (err, response) => {
            if (err) {
